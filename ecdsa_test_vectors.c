...
int run(Test* tests, int num_tests, uECC_Curve curve) {
    ...
    for (i = 0; i < num_tests; ++i) {
        strtobytes(tests[i].private_key, private_key, private_key_size);
        strtobytes(tests[i].public_key, public_key, public_key_size);
        ...
        result = uECC_sign_with_k(private_key, hash, private_key_size, k, signature, curve);
        if (!result) {
            all_success = 0;
        ...
        if (result) {
            ...
            result = uECC_verify(public_key, hash, private_key_size, signature, curve);
            if (!result) {
